...
describe('bootstrap', function() {
	describe('manual', function() {
		...
		it('compiles the element', function() {
			...
			window.angular.module('myModule', []).directive('myDirective', function() {
				return {
					compile: compileSpy
				};
			});
		...
		it('links the element', function() {
			...
			expect(linkSpy.calls.mostRecent().args[0]).toEqual(element.data('$injector').get('$rootScope'));
		});
		...
		it('supports enabling strictDi mode', function() {
			var element = $('<div><div my-directive></div></div>');
			var compileSpy = jasmine.createSpy();
			window.angular.module('myModule', []).constant('aValue', 42).directive('myDirective', function(aValue) {
				return {};
			});
			expect(function() {
				window.angular.bootstrap(element, ['myModule'], {
